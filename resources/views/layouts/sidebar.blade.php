...
<nav class="mt-4">
    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false" id="sidebarnav">
        ...
        @if (true)
        <li class="nav-item">
            <a href="{{ route('admin.saloon.index') }}" class="nav-link {{ request()->is('account/saloon*') ? 'active' : '' }}">
                <i class="nav-icon fa fa-user"></i>
                <p>
                    @lang('menu.saloon')
                </p>
        ...
        @if (true)
        <li class="nav-item">
            <a href="{{ route('admin.saloon_availability.index') }}" class="nav-link {{ request()->is('account/saloon_availability*') ? 'active' : '' }}">
                <i class="nav-icon fa fa-user"></i>
                <p>
                    @lang('menu.availability')
                </p>
            </a>
        </li>
        @endif 
        @if (true)
        <li class="nav-item">
            <a href="{{ route('admin.slot_booking.index') }}" class="nav-link {{ request()->is('account/slot_booking*') ? 'active' : '' }}">
                <i class="nav-icon fa fa-user"></i>
                <p>
                    @lang('menu.slot_booking')
                </p>
            </a>
        </li>
        @endif 
        ...
        <!----><!----><!----><!----><!--
        <!-- @if ($user->roles()->withoutGlobalScopes()->first()->hasPermission('create_point_of_sale'))
        <li class="nav-item">
        ...
        @endif -->
        @if ($user->roles()->withoutGlobalScopes()->first()->hasPermission('read_booking'))
        <li class="nav-item">
